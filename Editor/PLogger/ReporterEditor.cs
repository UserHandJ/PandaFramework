...
{
    //[MenuItem("UPandaGF/日志系统/创建运行时日志显示")]
...
public class ReporterModificationProcessor : UnityEditor.AssetModificationProcessor
{
    ...
    {
        static bool canWriteBuildInfo = true;
        static BuildInfo()
        {
            if (canWriteBuildInfo) EditorApplication.update += Update;
        }
        ...
        static void Update()
        {
            if (!EditorApplication.isCompiling && isCompiling)
            {
                ...
                build_info.Close();
                Debug.Log($"BuildInfo Finish Compile! writePath:{info_path}");
            }
